...
### 5.5 P0 Patch：MinerU 默认解析引擎回退为 pipeline (2026-04-27)

**背景**：6 文件压力测试中，1 个小 PDF 正常完成并入库，5 个大 PDF 均由 MinerU 返回明确模型/张量错误。失败集中出现在 `hybrid-auto-engine` 视觉/混合链路，Luceon 调度、active-task、Sidecar 观测和终态收敛未发现重复提交或 completed 未入库类问题。

**收口决策**：

1. 将系统默认本地 MinerU backend 从 `hybrid-auto-engine` 调整为 `pipeline`，优先保证大批量教育 PDF 的稳定处理。
2. 设置页中将 `pipeline` 标记为“默认/稳定”，保留 `hybrid-auto-engine` 与 `vlm-auto-engine` 作为人工选择项，但不再作为默认推荐。
3. 本地运行态 `mineruConfig.localBackend` 已同步更新为 `pipeline`，后续普通页面上传默认走 pipeline。
4. 独立本地 MinerU 测试脚本默认 backend 同步改为 `pipeline`，避免验收脚本与产品默认口径不一致。

**验证**：`node --check server/upload-server.mjs`、`node --check server/test-local-mineru.mjs`、`npx tsc --noEmit`、`npm run build`、`node server/tests/worker-smoke.mjs` 均通过。
