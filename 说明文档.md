...
- [x] **P0: 防止重复 ParseTask 创建 (2026-04-23)**
    - 服务端：在 `upload-server.mjs` (POST `/tasks`) 增加幂等防重逻辑，按 `materialId` 检查活跃任务，存在冲突时返回 `409 Conflict (TASK_ALREADY_ACTIVE)`。
    - 前端：`AssetDetailPage.tsx` 增加提交状态锁定，并在检测到 409 冲突时弹出友好提示及任务跳转链接。
    - 验收：新增 `uat/tests/idempotency.spec.ts` 自动化回归测试，验证并发/重复触发下的系统稳定性。
